...
export default function WaitingTimeLineCard({data}) {
...
const changes= action_data_keys.map((key)=>{
  ...
  if(key === "table_numbers"){
   after = data.waiting_list?.tables?.map((table)=>table.table?.table_number).filter(Boolean).join(", ") || "-"
   before = data.action_data[key] || "-"
  } else if(key === "start_time" || key === "end_time"){
    after = convertTimeZoneReservationDateTime(data.waiting_list[key])
    before = convertTimeZoneReservationDateTime(data.action_data[key])
  }
else{
  after = data.waiting_list[key] 
  before = data.action_data[key]
  }
  return (
