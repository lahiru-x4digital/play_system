...
export default function ProductExtraSettings() {
  const user = useSessionUser();
  const { extraHoursList, extraHoursListLoading, extraHoursListRefresh } =
    useGetExtraHours();
  ...
  return (
    <div>
      <div className="flex items-end gap-4 my-2">
        ...
        <CreateExtraPricingDialog
          onSuccess={() => {
            extraHoursListRefresh({ branch_id: selectedBranch });
          }}
        />
      </div>
      <ExtraHoursPricingTable
        data={extraHoursList}
        onRefresh={() => {
          extraHoursListRefresh({ branch_id: selectedBranch });
        }}
