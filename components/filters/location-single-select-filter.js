 ...
 */
export const LocationSingleSelectFilter = ({
  userType,
  userBranchId,
  onSearch,
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        if (initialBranch && (!showOnlyForSuperadmin || userType === 'SUPERADMIN' || userType === 'ORGANIZATION_USER')) {
          setLoading(true);
        ...
        } else if (initialCountry && (!showOnlyForSuperadmin || userType === 'SUPERADMIN' || userType === 'ORGANIZATION_USER')) {
          setLoading(true);
          // First load countries
          const countriesResponse = await discountService.getCountries();
          if (countriesResponse.success && Array.isArray(countriesResponse.data)) {
  ...
  const handleSearch = () => {
    ...
    try {
      if (!selectedBranch && !selectedBrand && !selectedCountry) {
        toast({
          variant: "destructive",
          title: "Error",
      ...
      if (onSearch && typeof onSearch === 'function') {
        const selectedCountryObj = countries.find(country => country.id.toString() === selectedCountry);
        onSearch({
          countryId: selectedCountryObj?.country_id,
...
useEffect(() => {
  handleSearch();
}, [selectedCountry, selectedBrand, selectedBranch]);
      ...
      <CardContent className="pt-6">
        <div className="flex flex-col md:flex-row gap-4 items-end">
          ...
          {/* <Button
            ...
            disabled={!selectedBranch && !selectedBrand && !selectedCountry || loading}
...
export default LocationSingleSelectFilter;