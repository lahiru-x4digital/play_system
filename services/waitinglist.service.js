...
export const waitingListService = {
  ...
  async updateWaitingList(id, updateData) {
    ...
    try {
      // console.log('Updating waiting list entry with data:', updateData)

      // Format the data according to the API requirements
      const formattedData = {
        ...(updateData.status && { status: updateData.status }),
        ...(updateData.notes && { notes: updateData.notes }),
        ...(typeof updateData.party_size === 'number' ? { party_size: updateData.party_size } : {}),
        ...(typeof updateData.archive === 'boolean' ? { archive: updateData.archive } : {}),
        // Only include table_ids if it's an array and not empty
        ...(Array.isArray(updateData.table_ids) && { table_ids: updateData.table_ids.filter(id => id !== undefined && id !== null) })
      }

      const response = await api.put(`/booking/waiting-list?id=${id}`, formattedData)
